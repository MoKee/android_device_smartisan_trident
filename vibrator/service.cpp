...
const static std::string kPlayPath = "/sys/class/haptic/player/play";

int main() {
    std::ofstream play(kPlayPath);
    if (!play) {
        LOG(ERROR) << "Failed to open " << kPlayPath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }

    android::sp<IVibrator> vibrator = new Vibrator(std::move(play));
